...
target_folder= 'C:\\Users\\gagni\\Pictures'
...
print(extensions)
#prints unique list of extensions that I have in my folder currently 

# create folders for each extension type that is in the folder 
for extension in extensions:
    if not os.path.exists(os.path.join(target_folder, extension)):
        os.mkdir(os.path.join(target_folder, extension))
#move files to their tolder
for item in os.listdir(target_folder):
    if os.path.isfile(os.path.join(target_folder,item)):  #move files not folders
           file_extension= item.split('.')[-1]
           shutil.move(os.path.join(target_folder, item), os.path.join(target_folder, file_extension, item)) #move to folder by file type and file name (item)
